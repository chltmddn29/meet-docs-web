...
// 회의가 삭제된 고아 음성(meetingId == null)이면 새 회의를 만들어 연결한 뒤 이동한다.
Future<void> regenerateMinutes(
  BuildContext context,
  WidgetRef ref,
  AudioFile audio,
) async {
  var meetingId = audio.meetingId;
  try {
    if (meetingId == null) {
      final res = await ref
          .read(dioProvider)
          .post(ApiConstants.newMeetingFromAudio(audio.transcriptId));
      meetingId = res.data['meeting_id'] as int;
      ref.invalidate(audioFilesProvider);
      ref.invalidate(meetingsProvider);
    }
    if (context.mounted) context.go('/result/$meetingId');
  } catch (e) {
    if (context.mounted) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('회의록 생성 준비 실패: $e')));
    }
  }
}
